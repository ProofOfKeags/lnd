...
func DeriveCommitmentKeys(commitPoint *btcec.PublicKey,
	commitType lntypes.ChannelParty, chanType channeldb.ChannelType,
...
func (cb *CommitmentBuilder) createUnsignedCommitmentTx(ourBalance,
	theirBalance lnwire.MilliSatoshi, commitType lntypes.ChannelParty,
	...
	for _, htlc := range filteredHTLCView.ourUpdates {
		if HtlcIsDust(
			cb.chanState.ChanType, false, commitType, feePerKw,
	...
	for _, htlc := range filteredHTLCView.theirUpdates {
		if HtlcIsDust(
			cb.chanState.ChanType, true, commitType, feePerKw,
			htlc.Amount.ToSatoshis(), dustLimit,
		) {
	...
	for _, htlc := range filteredHTLCView.ourUpdates {
		...
		err := addHTLC(
			commitTx, commitType, false, htlc, keyRing,
	...
	for _, htlc := range filteredHTLCView.theirUpdates {
		...
		err := addHTLC(
			commitTx, commitType, true, htlc, keyRing,
			cb.chanState.ChanType,
		)
		if err != nil {
			return nil, err
...
func genSegwitV0HtlcScript(chanType channeldb.ChannelType,
	isIncoming bool, commitType lntypes.ChannelParty, timeout uint32,
	...
	// script.
	case isIncoming && commitType == lntypes.Local:
	...
	case isIncoming && commitType == lntypes.Remote:
...
// channel.
func genTaprootHtlcScript(isIncoming bool, commitType lntypes.ChannelParty,
	...
	case isIncoming && commitType == lntypes.Local:
		htlcScriptTree, err = input.ReceiverHTLCScriptTaproot(
			timeout, keyRing.RemoteHtlcKey, keyRing.LocalHtlcKey,
			keyRing.RevocationKey, rHash[:], commitType,
		)
...
func genHtlcScript(chanType channeldb.ChannelType, isIncoming bool,
	commitType lntypes.ChannelParty, timeout uint32, rHash [32]byte,
	keyRing *CommitmentKeyRing,
	) (input.ScriptDescriptor, error) {

	if !chanType.IsTaproot() {
		return genSegwitV0HtlcScript(
			chanType, isIncoming, commitType, timeout, rHash,
			keyRing,
		)
	}

	return genTaprootHtlcScript(
		isIncoming, commitType, timeout, rHash, keyRing,
	)
}
...
func addHTLC(commitTx *wire.MsgTx, commitType lntypes.ChannelParty,
	...
	scriptInfo, err := genHtlcScript(
		chanType, isIncoming, commitType, timeout, rHash, keyRing,
	)
	if err != nil {
		return err
	}
